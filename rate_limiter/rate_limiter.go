...
// Interface for a thread-safe token bucket rate limiter.
type RateLimiter interface {
	// This returns the token bucket's maximum capacity.
	MaxQuota() float64

	// This sets the token bucket's maximum capacity.  The value must be non-negative.
	// Zero means "throttle everything". Unlimited means "throttle nothing".
	SetMaxQuota(q float64) error

	// This returns the token bucket's fill rate.
	QuotaPerSec() float64

	// This sets the token bucket's fill rate.  The value must be non-negative.
...
// This sets the token bucket's maximum capacity.  The value must be
// non-negative.
...
// returns true.
//
// NOTE: When maxQuota is Unlimited, or when the rate limiter is stopped,
// this returns immediately.
