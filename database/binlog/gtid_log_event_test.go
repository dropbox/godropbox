...
func (s *GtidLogEventSuite) TestCommitGtid57(c *C) {
	data := &bytes.Buffer{}
	// commit
	data.WriteByte(1)
	// sid
	...
	data.WriteByte(2) // logicalTimestampTypeCode

	data.Write([]byte{1, 0, 0, 0, 0, 0, 0, 0})

	data.Write([]byte{2, 0, 0, 0, 0, 0, 0, 0})
	s.WriteEvent(
		mysql_proto.LogEventType_GTID_LOG_EVENT,
		uint16(0),
		data.Bytes())
	...
	c.Assert(gle.Gno(), Equals, uint64(1))
	c.Assert(gle.lastCommited, Equals, int64(1))
	c.Assert(gle.sequenceNumber, Equals, int64(2))
}
